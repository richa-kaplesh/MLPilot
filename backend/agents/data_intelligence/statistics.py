...
def detect_target_column(df: pd.DataFrame, problem_statement: str) -> str:
    llm = Groq(api_key=settings.groq_api_keys[0])

    prompt = f"""
Given the problem statement: "{problem_statement}"
And these column names : {list(df.columns)}

Which column is most likely the target variable to predict?
